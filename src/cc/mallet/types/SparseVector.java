...
public class SparseVector implements ConstantMatrix, Vector, Serializable
{
    ...
    */
    ...
    @Override public int getNumDimensions () { return 1; }
    
    // xxx What do we return for the length?  It could be higher than this index.
    @Override public int getDimensions (int[] sizes)
    {
        if (indices == null) {
    ...
    @Override public int numLocations () {
        return (values == null ? (indices == null ? 0 : indices.length) : values.length);
    }
    ...
    @Override public double valueAtLocation (int location) { return values == null ? 1.0 : values[location]; }
    @Override public int indexAtLocation (int location) { return indices == null ? location : indices[location]; }
    ...
    @Override public double value (int index) {
        if (indices == null) {
            try {
                return values[index];
    ...
    @Override public int singleIndex (int[] indices) {
        assert (indices.length == 1); return indices[0];
    }
    
    @Override public void singleToIndices (int i, int[] indices) { indices[0] = i; }
    
    @Override public double singleValue (int i) { return value(i); }
  ...
  /***********************************************************************
    ...
    public final void map (Method f) throws IllegalAccessException, Throwable {
        ...
        try {
            for (int i=0 ; i<values.length ; i++) {
                // Convert double to Double and apply
