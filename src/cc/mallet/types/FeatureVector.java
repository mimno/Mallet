...
public class FeatureVector extends SparseVector implements Serializable, AlphabetCarrying {
    ...
    private static int[] indicesWithConjunctions (FeatureVector fv, Alphabet newVocab, FeatureSelection fsNarrow, FeatureSelection fsWide) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            for (int j = 0; j < length; j++) {
                if ((fsWide == null || fsWide.contains (fv.indices[j])) && fv.indices[i] != fv.indices[j]
    ...
    * (presumably more compact, dense) Alphabet. */
    ...
    @Override public ConstantMatrix cloneMatrixZeroed () {
        assert (values != null);
        if (indices == null)
            ...
            return new FeatureVector (dictionary, newIndices, new double[values.length], values.length, values.length, false, false, false);
        }
    }
    ...
    @Override public String toString (boolean onOneLine) {
        //Thread.currentThread().dumpStack();
        StringBuilder sb = new StringBuilder ();
        //System.out.println ("FeatureVector toString dictionary="+dictionary);
        if (values == null) {
