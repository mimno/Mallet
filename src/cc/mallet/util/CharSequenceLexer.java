...
public class CharSequenceLexer implements Lexer, Serializable
{
    ...
    {
        if (!regex.pattern().equals( getPattern() )) {
...
//            this.matcher = regex.matcher(input);
    ...
    @Override public String getTokenString ()
    {
        return matchText;
    }
    ...
    {
        try {
            //BufferedReader in = new BufferedReader(new FileReader(args[0]), Charset.defaultCharset());

            BufferedReader in = Files.newBufferedReader(Paths.get(args[0]), Charset.defaultCharset());
