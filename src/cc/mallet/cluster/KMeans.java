	...
	public Clustering cluster(InstanceList instances) {
		...
		while (deltaMeans > MEANS_TOLERANCE && iterations < MAX_ITER && deltaPoints > instances.size() * POINTS_TOLERANCE) {
			...
			for (int n = 0; n < instances.size(); n++) {
				...
				for (int c = 0; c < numClusters; c++) {
					instDist = metric.distance( (SparseVector)clusterMeans.get( c ), (SparseVector)instances.get( n ).getData() );
				...
				((InstanceList)instanceClusters.get( instClust )).add( instances.get( n ) );
						...
						case EMPTY_SINGLE:
							...
							for (int clusters = 0; clusters < clusterMeans.size(); clusters++) {
								...
								for (int n = 0; n < centInstances.size(); n++) {
									double currentDist = metric.distance( centroid, (SparseVector)centInstances.get( n ).getData() );
							...
							else clusterMeans.set( c, cacheList.get( newCentroid ).getData() );
