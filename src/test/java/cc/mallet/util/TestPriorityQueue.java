...
public class TestPriorityQueue {
  ...
  public void testReverse() {
    ...
    while (q.size() > 0) {
      ...
      pr_last = e.getPriority();
      assertEquals("ascending priority", items[j].getPriority(), e.getPriority(), 1e-5);
  ...
  public void testEqualKeys() {
    ...
    for (int i = 0; i < 5; i++) {
      ...
      assertEquals(3.0, e.getPriority(), 1e-5);
    }
    for (int i = 0; i < 5; i++) {
      QueueElement e = q.extractMin();
      assertTrue(q.contains(q.min()));
      assertEquals(4.0, e.getPriority(), 1e-5);
    }
    for (int i = 0; i < 5; i++) {
      QueueElement e = q.extractMin();
      assertTrue(q.contains(q.min()));
      assertEquals(5.0, e.getPriority(), 1e-5);
    }
    for (int i = 0; i < 5; i++) {
      QueueElement e = q.extractMin();
      if (q.size() > 0)
        assertTrue(q.contains(q.min()));
      assertEquals(7.0, e.getPriority(), 1e-5);
    }
  }
